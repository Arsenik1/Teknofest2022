...
# Constants
PERCENT_CENTER_RECT  = 0.17 # For calculating the center rectangle's size
...
if __name__ == "__main__":
    ...
    t2.start()
    t3.start()
    waterPool = LocationGlobalRelative(40.8118285,29.3604422,2.7)
    waterPoolHigh = LocationGlobalRelative(40.8118285,29.3604422,7)
    cmds = drone.commands
    cmds.clear() 
    ...
    while True:
        take_water(0,0,0.2)
        if drone.location.global_relative_frame.alt <= 2.55:
            break
    take_water(0,0,0)
    time.sleep(1)
    ...
    while True:
        take_water(0,0,0.5)
        if drone.location.global_relative_frame.alt <= 3.5:
            break
    take_water(0,0,0.2)
