...
def get_target_coordinates(vid_cam,redOrBlue):
    ...
    if redOrBlue == 1:
        HSV_LOWER_BOUND = (80, 95, 20) #yeni olcum: 80, 15, 20
        HSV_UPPER_BOUND = (120, 255, 255)#yeni olcum: 120, 255, 255
...
if __name__ == "__main__":
    ...
    waterPool = LocationGlobalRelative(-35.3631997,149.1655049,10)
    cmds = drone.commands
    ...
    while True:
        nextwaypoint=drone.commands.next
        if nextwaypoint == 2: #3 olmasi gerekmiyor mu#########
            break
        time.sleep(4)
    t7.start()
    t8.start()
    ...
    cmds.clear()
    processflag=False
    addWaypoint(cmds,-35.3631232,149.1655692,HOVERING_ALTITUDE)#dummy
    addWaypoint(cmds,-35.3631232,149.1655692,HOVERING_ALTITUDE)#1
    addWaypoint(cmds,-35.3630248,149.1654003,HOVERING_ALTITUDE)#2
    ...
    while True:
        take_water(0,0,0.4) #0.2 before
        print(drone.location.global_relative_frame.alt)
        if drone.location.global_relative_frame.alt <= 5.00:
            break
    take_water(0,0,0)
    processflag = True
    redAreaTime=False
    ...
    time.sleep(1)
    addWaypoint(cmds,-35.3630215, 149.1651991,HOVERING_ALTITUDE)#kirmizi sonu #dummy
    addWaypoint(cmds,-35.3630215, 149.1651991,HOVERING_ALTITUDE)#kirmizi sonu
    addWaypoint(cmds,-35.3631254, 149.1650234,HOVERING_ALTITUDE)#direk hiza
    addWaypoint(cmds,-35.3632085, 149.165301,HOVERING_ALTITUDE)#inis
    addWaypoint(cmds,-35.3632085, 149.165301,HOVERING_ALTITUDE)#inis #dummy
    # Reset mission set to first (0) waypoint
