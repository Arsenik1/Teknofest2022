...
HOVERING_ALTITUDE    = 10.0 # Altitude in meters to which the drone will perform its tasks
...
def main(x):
    global N_coord,E_coord,D_coord,yaw_angle,vx,vy,detected,rectflag,redAreaCenter,frame,tgtcounter
    ...
    if(x == 0):
        print("her seyi kontrol et: servo drop, mission yazma, kodu yukleme")
        time.sleep(5)
    ...
    while x == 1:
        ...
        if tgt_cam_coord['width'] is not None and tgt_cam_coord['height'] is not None and tgtcounter != 1:
...
def goster():
    while True:
        ...
        time.sleep(0.0001)
def measureTime():
    global redAreaTime,measureTimeSecond,waterPourFlag
    while True:
        start = time.time()
        while detected == True and rectflag == True :
            if waterPourFlag == True and time.time() - start > 1.80:
                print("Water is pouring...")
                suyuBirak()
            if time.time() - start > measureTimeSecond:
                redAreaTime = True
                break
            time.sleep(0.15)
...
def get_target_coordinates(vid_cam,redOrBlue):
    ...
    if redOrBlue == 1:
        HSV_LOWER_BOUND = (92, 81, 30) #yeni olcum: 92, 120(81), 30 //// 72, 52, 102 (calisiyor) //// 80, 95, 20 (calismiyor)
        HSV_UPPER_BOUND = (108, 255, 255)#yeni olcum: 108, 255, 255 //// 110, 255, 255 (calisiyor) //// 120, 255, 255 (calismiyor)
...
if __name__ == "__main__":
    ...
    t10 = threading.Thread(target = measureTimeTwo)
    t11 = threading.Thread(target = set_position)
    t12 = threading.Thread(target = main,args = (1,))
    t13 = threading.Thread(target=measureTime)
    waterPool = LocationGlobalRelative(38.7896705, 30.4827619,10) #yukseklik 10 dikkat et
    cmds = drone.commands
    ...
    while True:
        ...
        if nextwaypoint==5 and processflag != True :#Skip to next waypoint
            ...
            drone.groundspeed = 6.00
            print("Red found")
            drone.commands.next = 6
        if nextwaypoint==9: #Dummy waypoint - as soon as we reach waypoint 4 this is true and we exit.
            print("Exit 'standard' mission when start heading to final waypoint (5)")
            break
        time.sleep(0.0001)
    #drone.groundspeed = 2.00
    drone.parameters['WPNAV_SPEED']=250
    drone.mode = VehicleMode("GUIDED")
    ...
    cmds.clear()
    processflag=False
    addWaypoint(cmds,38.7894703,30.4830645,HOVERING_ALTITUDE)#dummy
    addWaypoint(cmds,38.7894703,30.4830645,HOVERING_ALTITUDE)#1
    addWaypoint(cmds,38.7897570,30.4833184,HOVERING_ALTITUDE)#2
    ...
    while True:
        take_water(0,0,0.4) #0.2 before
        print(drone.location.global_relative_frame.alt)
        if drone.location.global_relative_frame.alt <= 5.15:
            break
    take_water(0,0,0)
    processflag = True
    redAreaTime=False
    ...
    drone.parameters['WPNAV_SPEED']=800
    time.sleep(1)
    addWaypoint(cmds,38.7901273, 30.4832083,HOVERING_ALTITUDE)#kirmizi sonu #dummy
    addWaypoint(cmds,38.7901273, 30.4832083,HOVERING_ALTITUDE)#kirmizi sonu
    addWaypoint(cmds,38.7903681, 30.4827856,HOVERING_ALTITUDE)#direk hiza
    addWaypoint(cmds,38.7899673, 30.4823075,HOVERING_ALTITUDE)#inis
    addWaypoint(cmds,38.7899673, 30.4823075,HOVERING_ALTITUDE)#inis #dummy
    # Reset mission set to first (0) waypoint
